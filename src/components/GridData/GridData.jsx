      ...
      <style
        dangerouslySetInnerHTML={{
          ...
          ${signatories
            .map(
              (_, index) => `
            .stagger-item:nth-child(${index + 1}) { 
              animation-delay: ${0.1 + index * 0.01}s; 
            }
          `
            )
            .join("")}
      ...
      <div className="flex flex-col items-center justify-center min-h-screen px-4">
        <div
          ...
          className={`text-center mb-8 ${
            isVisible ? "counter-animate" : "opacity-0 translate-y-5"
          }`}
        >
      ...
      <div className="px-4 pb-16">
        <div
          className={`max-w-7xl mx-auto ${
            isVisible ? "signatories-animate" : "opacity-0"
          }`}
        >
          <div className="columns-2 sm:columns-3 md:columns-4 lg:columns-5 xl:columns-6 gap-4 space-y-1">
            {signatories.map((signatory, index) => (
              <div
                key={`${signatory.name}-${index}`}
                className="break-inside-avoid stagger-item"
              >
                <a
                  href={signatory.url}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="block text-sm text-gray-400 signatory-link py-1"
                  aria-label={`Visit ${signatory.name}'s website`}
                >
                  {signatory.name}
                </a>
              </div>
